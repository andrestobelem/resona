...
touch unrelated lock entries. `skills status` is read-only and reports `missing`,
...
present in the project. It refuses to overwrite a locally modified skill unless the caller
passes `--force`; the flag is the only authorization for that replacement. Successful
